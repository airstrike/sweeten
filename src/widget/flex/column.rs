//! A vertical flex container — CSS-flex `Column`.
//!
//! Distributes its children along the main (vertical) axis using the
...
///
/// Mirrors CSS `display: flex; flex-direction: column` semantics —
/// the main axis is vertical, the cross axis is horizontal, and each
/// child can carry its own `flex-grow`, `flex-shrink`, `flex-basis`,
/// and `align-self` via [`FlexChild`].
///
    ...
    Renderer: crate::core::Renderer,
{
    ...
    /// Creates a [`Column`] from an iterator of plain [`Element`] children.
    ///
    ...
    /// Creates a [`Column`] from an iterator of [`FlexChild`] children.
    ...
    /// Creates a [`Column`] from an already allocated [`Vec`] of
    /// [`FlexChild`].
    pub fn from_vec(
        children: Vec<FlexChild<'a, Message, Theme, Renderer>>,
    ) -> Self {
    ...
    /// Sets the spacing between adjacent items along the main axis.
    ///
    /// Alias of [`Column::spacing`] using the CSS `gap` name.
    ...
    ///
    /// Mirrors CSS `flex-direction: column-reverse`. The first source
    /// child appears at the visual bottom edge, the last at the top.
    ...
    /// Pushes a plain [`Element`] onto the [`Column`].
    ///
    ...
    /// Pushes an explicit [`FlexChild`] onto the [`Column`].
    pub fn push_flex(
        mut self,
    ...
    /// Pushes a plain element onto the [`Column`] when `child` is
    /// [`Some`], leaving the column unchanged otherwise.
    pub fn push_maybe(
        self,
    ...
    /// Pushes a [`FlexChild`] onto the [`Column`] when `child` is
    /// [`Some`], leaving the column unchanged otherwise.
    ...
    /// Extends the [`Column`] with the given plain element children.
    pub fn extend(
        self,
        children: impl IntoIterator<Item = Element<'a, Message, Theme, Renderer>>,
    ) -> Self {
    ...
    /// Extends the [`Column`] with the given [`FlexChild`] children.
    pub fn extend_flex(
        self,
        children: impl IntoIterator<Item = FlexChild<'a, Message, Theme, Renderer>>,
    ) -> Self {
...
/// Creates a flex [`Column`] from a comma-separated list of children.
///
...
/// (e.g. `sweeten::flex_column!`); the canonical path is
/// `sweeten::widget::flex::column!` via the in-module re-alias just
/// below this macro.
#[macro_export]
macro_rules! flex_column {
    () => (
        $crate::widget::flex::Column::new()
    );
    ($($x:expr),+ $(,)?) => (
        $crate::widget::flex::Column::with_flex_children(
...
mod tests {
    ...
    /// `crate::flex_column!`, `flex::column!`, `flex::column(...)`,
    /// and direct `Column::new()` — so the three-namespace re-alias
    /// at `widget::flex::column` is covered. Layout correctness is
    /// the engine tests' job.
    #[test]
    fn _compiles() {
        // `crate::flex_column!` — `#[macro_export]` form at the crate root.
        let _root_macro: Column<'_, (), Theme, Renderer> = crate::flex_column![
            text("a"),
        ...
        // `flex::column!` — re-aliased macro at the canonical module path.
        let _aliased_macro: Column<'_, (), Theme, Renderer> = crate::widget::flex::column![
            text("x"),
            flex(text("y")).shrink(0.0)
        ];

        // `flex::column(...)` — free function constructor.
        let _from_fn: Column<'_, (), Theme, Renderer> =
            crate::widget::flex::column([text("a").into(), text("b").into()]);

        // Empty macro variant.
        let _empty: Column<'_, (), Theme, Renderer> = crate::flex_column![];

        // Direct constructor + push_maybe_flex / push_maybe variants.
        let none_child: Option<Element<'_, (), Theme, Renderer>> = None;
        let _direct: Column<'_, (), Theme, Renderer> = Column::new()
            .push(text("z"))
